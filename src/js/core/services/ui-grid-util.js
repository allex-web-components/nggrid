(function() {
  ...
  module.service('gridUtil', ['$log', '$window', '$document', '$http', '$templateCache', '$timeout', '$interval', '$injector', '$q', '$interpolate', 'uiGridConstants',
    ...
    var elementWidth = elWidthOrHeight.bind(null, 'width'),
      elementHeight = elWidthOrHeight.bind(null, 'height'),
      outerElementWidth = elOuterWidthOrHeight.bind(null, 'width'),
      outerElementHeight = elOuterWidthOrHeight.bind(null, 'height');

    function deCapitalize (match) {
      return angular.lowercase(angular.uppercase(match.charAt(0)) + match.slice(1));
    }
    function capitalizeFirst (match) {
      return angular.uppercase(match.charAt(0)) + match.slice(1);
    }

    function excludeProps (excludeProperties, columnDefs, prop, propName) {
    ...
    function templateLoadSuccess (template, result) {
    ...
    function templateLoadFailed  (template, err) {
    ...
    function doesContainObjWithProp (propertyName, propertyValue, prev, object){
      return prev || (object[propertyName] === propertyValue);
    }

    function focusElement (id, Grid) {
      var elementID = (Grid && Grid.id ? Grid.id + '-' : '') + id;
      var element = $window.document.getElementById(elementID);
      if (element) {
        element.focus();
      } else {
        logWarn('[focus.byId] Element id ' + elementID + ' was not found.');
      }
    }
    ...
    function elWidthOrHeight (name, elem, extra) {
      var e = elem;
      ...
      if (e) {
        ...
        return e.offsetWidth === 0 && rdisplayswap.test(styles.display) ?
                  swap(e, cssShow, getWidthOrHeight.bind(null, e, name, extra)) :
    ...
    function elOuterWidthOrHeight (name, elem, margin) {
      return elem ? elWidthOrHeight(name, elem, margin ? 'margin' : 'border') : null;
    }

    function appendPreparsed (preparsed, part){
    ...
    Debounce.prototype.go = function (context, args) {
       var callNow = this.immediate && !this.timeout;
       if (this.timeout) {
         this.cancel();
       }
       this.timeout = $timeout(this.later.bind(this, context, args), this.wait, false);
       if (callNow) {
         this._itsTime(context, args);
       }
       //what should be returned? perhaps promise which will be resolved upon debounce
    };

    Debounce.prototype._itsTime = function (context, args) {
      this.func.apply(context, args);
    };

    Debounce.prototype.later = function (context, args) {
      this.timeout = null;
      if (!this.immediate) {
        this._itsTime(context, args);
      }
    ...
    var on = {
     mousewheel : function(elm, fn) {
      ...
      $elm.data('mousewheel-page-height', elementHeight($elm));
    ...
    function doOff (eventName, events, elm, fn){
      var idx = events[eventName].indexOf(fn);
      if (idx > 0) {
        events[eventName].removeAt(idx);
      }
    }
    ...
    var s = {
      ...
      readableColumnName: function (columnName) {
        ...
        return columnName.replace(/_+/g, ' ')
          ...
          .replace(/^[A-Z]+$/, deCapitalize)
          // Capitalize the first letter of words
          .replace(/([\w\u00C0-\u017F]+)/g, capitalizeFirst)
      ...
      getColumnsFromData: function (data, excludeProperties) {
        ...
        var item = data[0];
        angular.forEach(item, excludeProps.bind(null, excludeProperties, columnDefs));
        return columnDefs;
      },
      ...
      getTemplate: function (template) {
        ...
        if ($templateCache.get(template)) {
          return postProcessTemplate($templateCache.get(template));
        }

        // See if the template is itself a promise
        if (angular.isFunction(template.then)) {
          return template.then(postProcessTemplate);
        }

        // If the template is an element, return the element
        try {
          if (angular.element(template).length > 0) {
            return $q.when(template).then(postProcessTemplate);
          }
        }
        catch (err){
        ...
        return $http({ method: 'GET', url: template})
          .then(templateLoadSuccess.bind(null, template), templateLoadFailed.bind(null, template))
          .then(postProcessTemplate);
      },
    ...
    /**
      ...
      */
      ...
      arrayContainsObjectWithProperty: function(array, propertyName, propertyValue) {
        return array.reduce(doesContainObjWithProp.bind(null, propertyName, propertyValue), false);
      },
      ...
      closestElm : function (el, selector) {
      ...
      getBorderSize : function getBorderSize(elem, borderType) {
      ...
      rtlScrollType : function rtlScrollType() {
       ...
       */
      normalizeScrollLeft : function normalizeScrollLeft(element, grid) {
      ...
      */
      denormalizeScrollLeft : function denormalizeScrollLeft(element, scrollLeft, grid) {
        if (typeof(element.length) !== 'undefined' && element.length) {
          element = element[0];
        }
      ...
      preEval : function (path) {
        ...
        if (m) {
          ...
          angular.forEach(parts, appendPreparsed.bind(null, preparsed));
      ...
      debounce : function (func, wait, immediate) {
        ...
        var dobj = new Debounce(func, wait, immediate);
        return function () {
          return dobj.go(this, arguments);
        };
      },
      ...
      off: off,
      _events : _events,
      addOff : function (eventName) {
        off[eventName] = doOff.bind(null, eventName, _events);
      },
      outerElementWidth: outerElementWidth,
      outerElementHeight: outerElementHeight
    };
    ...
    s.focus = {
      ...
      byId: function (id, Grid) {
        this._purgeQueue();
        var promise = $timeout(focusElement.bind(null, id, Grid));
      ...
      byElement: function(element){
        if (!angular.isElement(element)){
          logWarn("Trying to focus on an element that isn\'t an element.");
          return $q.reject('not-element');
        }
        element = angular.element(element);
        this._purgeQueue();
        var promise = $timeout(doFocus);
        this.queue.push(promise);
        return promise;
      },
      /**
      ...
      bySelector: function(parentElement, querySelector, aSync){
        ...
        this._purgeQueue();
        var focusBySelector = focusBySelectorF.bind(null, this, parentElement, querySelector);
